...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    private void generateAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        ...
        if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_REVEAL) {
        	Log.e("generateAnimate", "SWIPE_ACTION_REVEAL");
            generateRevealAnimate(true, view, swap, swapRight, position);
        }
        if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_DISMISS) {
        	Log.e("generateAnimate", "SWIPE_ACTION_DISMISS");
        ...
        if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_CHOICE) {
        	Log.e("generateAnimate", "SWIPE_ACTION_CHOICE");
    ...
    /**
     * Create Reveal no animation
     *
     * @param frontView      affected front view
     * @param backView		affected back view
     * @param swap      If will change state. If is "false" returns to the original position
     * @param position  Position of list
     */
    private void generateRevealNoAnimate(
    	...
    	if (swap) {
    		if (opened.get(position)) {
            	...
            	Log.e("generateRevealNoAnimate", "show back view");
     ...
     * @param position  list position
     */
    private void generateRevealAnimate(final boolean isResetScrolling, final View view, final boolean swap, final boolean swapRight, final int position) {
        int moveTo = 0;
        if (opened.get(position)) {
            if (!swap) {
                moveTo = openedRight.get(position) ? (int) (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset);
    ...
    private void closeAllOpenedItem(){
    	if (opened != null) {
            int start = swipeListView.getFirstVisiblePosition();
            int end = swipeListView.getLastVisiblePosition();
            for (int i = start; i <= end; i++) {
                if (opened.get(i)) {
                    generateRevealAnimate(false, swipeListView.getChildAt(i - start).findViewById(swipeFrontView), true, false, i);
                }
            }
        }
    }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (deltaMode > slop && swipeCurrentAction == SwipeListView.SWIPE_ACTION_NONE && velocityY < velocityX) {
                    ...
                    if (opened.get(downPosition)) {
                        ...
                        if (swipingRight && swipeActionRight == SwipeListView.SWIPE_ACTION_REVEAL_NO_ANIM) {
                            swipeCurrentAction = SwipeListView.SWIPE_ACTION_REVEAL_NO_ANIM;
                        } else if (!swipingRight && swipeActionLeft == SwipeListView.SWIPE_ACTION_REVEAL_NO_ANIM) {
                            swipeCurrentAction = SwipeListView.SWIPE_ACTION_REVEAL_NO_ANIM;
                        } else {
                        ...
                        if (swipingRight && swipeActionRight == SwipeListView.SWIPE_ACTION_DISMISS) {
                            ...
                            swipeCurrentAction = SwipeListView.SWIPE_ACTION_CHOICE;
                        } else if (swipingRight && swipeActionRight == SwipeListView.SWIPE_ACTION_REVEAL_NO_ANIM) {
                            swipeCurrentAction = SwipeListView.SWIPE_ACTION_REVEAL_NO_ANIM;
                        } else if (!swipingRight && swipeActionLeft == SwipeListView.SWIPE_ACTION_REVEAL_NO_ANIM) {
                            swipeCurrentAction = SwipeListView.SWIPE_ACTION_REVEAL_NO_ANIM;
                        } else {
